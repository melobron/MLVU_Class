...
from GOPRO_Large2 import GOPRO_Large_train_sharp
    ...
    def train(self, epoch, verbose):
        ...
        for i in range(epoch):
            ...
            for index, imgs in enumerate(self.dataloader):
                print('{}th epoch, {}th batch'.format(i, index))
            ...
            if (i + 1) % verbose == 0:
                ...
                matplotlib.image.imsave('Generated_Images2.jpg', gen_imgs.numpy())
...
if __name__ == "__main__":
    batch_size = 1
    img_size = 256
    data = GOPRO_Large_train_sharp(img_size=img_size)
    train_dataset, _ = train_test_split(data, train_size=800, shuffle=True)
    ...
    schedule_opt = {'schedule': 'linear', 'n_timestep': 500, 'linear_start': 1e-4, 'linear_end': 0.05}

    ddpm = DDPM(device, dataloader=dataloader, schedule_opt=schedule_opt,  # in_channel=1, out_channel=1,
                save_path='./ddpm2.pt', load_path='./ddpm2.pt', load=False, img_size=img_size, inner_channel=128,
                norm_groups=32, channel_mults=[1, 2, 2, 2], res_blocks=2, dropout=0.2, lr=5 * 1e-5, distributed=False)
    print('Train Start')
    ddpm.train(epoch=100, verbose=50)
